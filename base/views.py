...
def Group2(name, var, var2, diameter):
    ...
    if name == "Reducer":
        if var2 =='Big':
            return Reducer.objects.get(Q(pipe_diam_big = diameter)&(Q(small_pipe_diam1 = var)|Q(small_pipe_diam2 = var)|Q(small_pipe_diam3 = var)|Q(
                small_pipe_diam4 = var)|Q(small_pipe_diam5 = var)|Q(small_pipe_diam6 = var))).install_length_mm
        elif var2 =='Small':
            return Reducer.objects.get(Q(pipe_diam_big = var)&(Q(small_pipe_diam1 = diameter)|Q(small_pipe_diam2 = diameter)|Q(small_pipe_diam3 = diameter)|Q(
                small_pipe_diam4 = diameter)|Q(small_pipe_diam5 = diameter)|Q(small_pipe_diam6 = diameter))).install_length_mm
        else :
            return ''
    if name == 'ReducerDIN':
        if var2 == 'Big':
...
def test_Ajax(request):
    ...
    if compAname == '' and compBname =='' or compAweld == '' or compBweld == '':
        return JsonResponse({'reason' : 'no components have been chosen', "valid": False}, content_type="application/json")
    ...
    if compAweld != '':
        try:
            ...
            return JsonResponse({'reason':'cannot read weld of component A as a number', "valid": False}, content_type="application/json")
    else:
        compAweld = 0
    if compBweld != '':
        try :
            ...
            return JsonResponse({'reason':'cannot read weld of component B as a number', "valid": False}, content_type="application/json")
    else:
        compAweld = 0
    if compAname == '':
        compAweld = 0
    ...
    try:
        ...
        return JsonResponse({'reson':'cannot read diameter as a number', "valid": False}, content_type="application/json")
    try:
        length = float(request.GET.get("length", None))
    except ValueError:
        return JsonResponse({'reason':'cannot read length as a number', "valid": False}, content_type="application/json")
    if compAname =='SPECIAL':
        try:
            specialsizeA = float(request.GET.get('compAinput',None))
            print(specialsizeA)
        except ValueError:
            return JsonResponse({'reason':'cannot convert component A\' s inbuilt size to float', "valid": False}, content_type="application/json")
    if compBname == 'SPECIAL':
        try:
            specialsizeB = float(request.GET.get('compBinput',None))
        except ValueError:
            return JsonResponse({'reason':'cannot convert component B\' s inbuilt size to float', "valid": False}, content_type="application/json")
    inbs = 0
    inbs2 = 0
    ...
    if resp <= 0:
        return JsonResponse({'reason':'expected length is less than 0, please check again',"valid": False}, content_type="application/json")
...
def checkDia(request):
    ...
    if flag == "true":
        ...
        all_spieces = {'name1': 'Angle90', 'name2': 'Angle180', 'name3': 'TPieceDIN', 'name4': 'ReducerDIN', 'name5':'TReducerPieceDIN',
        ...
        if temp or temp2 or temp3 or temp4 or temp5 or temp6 or temp7 or temp8 or temp9 or temp10:
            ...
            return JsonResponse(
                {"valid": True, "data1": somedata4, "data2": somedata5, "data3": somedata6, "data4": somedata8, "data5": somedata2, "data6": somedata,
